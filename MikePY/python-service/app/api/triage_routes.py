...
async def predict_triage(request: Dict[str, Any]):
    logger.info(f"Prediccion de triaje solicitada")
    ...
    try:
        ...
        response = {
            ...
            "nivel_triage": result["nivel_triage"],
            "descripcion_triage": result["descripcion"],
...
async def train_triage():
    ...
    try:
        ...
        if len(all_cases) < 100:
            return {
                "status": "error",
                "message": f"Datos insuficientes: {len(all_cases)} casos"
            }

...
class TriageFeatureExtractor:
    ...
    def extract_features(self, triage_data: Dict[str, Any]) -> np.ndarray:
        ...
        for vital in _FIXED_VITALS:
            value = float(vital_signs.get(vital, 0.0))
        ...
        hr  = float(vital_signs.get("heart_rate_bpm", 0.0))
        rr  = float(vital_signs.get("respiratory_rate_bpm", 1.0))
        sbp = float(vital_signs.get("systolic_bp_mmhg", 0.0))
        dbp = float(vital_signs.get("diastolic_bp_mmhg", 0.0))
        spo2 = float(vital_signs.get("oxygen_saturation_pct", 95.0))

        features["hr_rr_ratio"] = hr / max(rr, 1)
        features["map"] = (sbp + 2 * dbp) / 3
        features["pulse_pressure"] = sbp - dbp
        features["hypoxia_severity"] = max(0.0, 95.0 - spo2)
